...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle Learn More buttonsfir
    const learnMoreButtons = document.querySelectorAll('button:not([onclick])');
    learnMoreButtons.forEach(button => {
        if (button.textContent === 'LEARN MORE' || button.textContent === 'Learn More') {
            button.addEventListener('click', function() {
                window.scrollTo({
                    top: document.getElementById('page2').offsetTop,
                    behavior: 'smooth'
                });
            });
        }
    ...
    // Handle Book Demo button in page13
    const bookDemoButton = document.querySelector('#page13 button');
    if (bookDemoButton) {
        bookDemoButton.addEventListener('click', function() {
            window.location.href = 'output/book-page.html';
        });
    }
});
